...
For the connector on the throttle pedal input **the smooth side is positive** and the sanded side is negative!
Must not plug it in the wrong way around, don't know what would happen.

And the same orientation for the hall effect sensor.

I want to give the superglue on the hall effect sensor a big longer to dry, but then I should be able to fit the hall sensor & magnet to
the tractor, then wire everything up and do some analogue plotting with Arduino IDE to see if it works, and then think about making the
3d-printed housing.
