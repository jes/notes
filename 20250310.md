 ...
 * chamfering: we should basically factor out smooth min code into a `smin()` function in `fragment.js`, and also add a similar one for chamfers, and use `if (abs(d1) < k && abs(d2) < k) return d1+d2-k; else return min(d1,d2);` as smoothmin but for chamfers
